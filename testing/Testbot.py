...
import random
import re
from collections import defaultdictge  # Assume this is your existing age verification function
from database import Database  # Assume this is your database module
...
@bot.tree.command(name="verify", description="Manually verify a user's age")
@commands.has_permissions(administrator=True)
async def manual_verify(interaction: discord.Interaction, member: discord.Member, age: int):
    await interaction.response.defer(ephemeral=True)
    
    if await db.is_verified(member.id):
        await interaction.followup.send(f"{member.name} is already verified.")
        return

    if age < 13:
        await interaction.followup.send(f"Sorry, {member.name} must be at least 13 years old to use this server.")
        return

    await db.add_user(member.id, age)
    await update_channel_permissions(member, age)

    await interaction.followup.send(f"{member.name} has been manually verified as {age} years old. Their channel access has been updated.")
    
    try:
        await member.send(f"You have been manually verified by an administrator. Your age has been set to {age}.")
    except discord.Forbidden:
        await interaction.followup.send(f"Couldn't send a DM to {member.name}. They may have DMs disabled.")

@bot.tree.command(name="update_age", description="Update a user's age and permissions")
@commands.has_permissions(administrator=True)
async def update_age(interaction: discord.Interaction, member: discord.Member, new_age: int):
    await interaction.response.defer(ephemeral=True)
    
    if not await db.is_verified(member.id):
        await interaction.followup.send(f"{member.name} is not verified yet. Use the verify command instead.")
        return

    if new_age < 13:
        await interaction.followup.send(f"Sorry, age must be at least 13.")
        return

    await db.update_user_age(member.id, new_age)
    await update_channel_permissions(member, new_age)

    await interaction.followup.send(f"{member.name}'s age has been updated to {new_age}. Their channel access has been updated.")
    
    try:
        await member.send(f"Your age has been updated to {new_age} by an administrator. Your channel access may have changed.")
    except discord.Forbidden:
        await interaction.followup.send(f"Couldn't send a DM to {member.name}. They may have DMs disabled.")
